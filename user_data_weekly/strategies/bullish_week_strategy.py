...
class WeeklyCandleStrategy(IStrategy):
    """
    ...
    def populate_entry_trend(
        ...
        # hammer: abs(high - close) < abs(open - low)
        dataframe.loc[
            (
                (
                    abs(dataframe["high"] - dataframe["close"])
                    < abs(dataframe["open"] - dataframe["low"])
                )
                or (
        ...
        if dataframe["enter_long"].iloc[-1] == 1:
            ...
            self.dp.send_msg(
                f"Weekly - {current_time}: Enter Long for {metadata['pair']} with {', '.join(candle_names)} candlestick pattern"
            )
        return dataframe
